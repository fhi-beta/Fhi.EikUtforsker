...
{
    public class KryptertRekvisisjonsmeldingV107Tjeneste : IMeldingsformatTjeneste
    {
        private readonly StoreName _storeName;
        private readonly StoreLocation _storeLocation;

        public KryptertRekvisisjonsmeldingV107Tjeneste(IOptions<EikUtforskerOptions> options)
        {
        ...
        public string ValiderJson(string kryptert)
        {
            try
            {
                return JsonSchemaHelper.ValiderJson(kryptert, "Eik107/kryptertrekvisisjonsmelding.schema.json",
                    new Dictionary<string, string>() { { "http://www.fhi.no/legemiddelregisteret/eik/kryptertrekvisisjonsmelding/felles.schema.json", "Eik107/felles.schema.json" } });
            }
            catch (Exception ex)
            {
                return ex.Message;
        ...
        public List<string> GetJsonValideringsfeil(string json)
        {
            try
            {
                return JsonSchemaHelper.GetJsonValideringsfeil(json, "Eik107/rekvisisjonsmelding.schema.json",
                    new Dictionary<string, string>() { { "http://www.fhi.no/legemiddelregisteret/eik/rekvisisjonsmelding/felles.schema.json", "Eik107/felles.schema.json" } });
            }
            catch (Exception ex)
            {
